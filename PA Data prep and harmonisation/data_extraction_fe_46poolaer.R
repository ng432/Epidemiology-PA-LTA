...
for (x in avd42[c(2:6, 8:11, 13, 16)])
{
  ...
  add[add==4] = 1
  add[add==5] = 0.55
  add[add==6] = 0.0 # not sure on this value, just leaving it for now for ease
...
for (x in avd46[rs_i])
{
  ...
  add[add==4] = 0.55
  add[add==3] = 0.22
  add[add=2] = 0.0 # not sure on this value, just leaving it for now for ease, tracked up from above
...
save(avd_all, file = 'K:/Behavioural/Nicholas Gregory/Early data analysis/PAdata_fe_46pool.Rdata')
