...
for (x in avd42[c(2:6, 8:11, 13, 16)])
{
...
rs_i = c(
  seq(which(coln46 == 'B10Q25A'), which(coln46 == 'B10Q25C'), by = 3),
  seq(which(coln46 == 'B10Q25E'), which(coln46 == 'B10Q25F'), by = 3),
  seq(which(coln46 == 'B10Q25L'), which(coln46 == 'B10Q25S'), by = 3),
...
save(avd_all, file = 'K:/Behavioural/Nicholas Gregory/Early data analysis/PAdata_fe.Rdata')
...
for (x in which(avd46$sum >= 5.5)) # looping through people who do ~7 days of exercise a week
{
  
  for (j in avd46[x,c(seq(which(coln46 == 'B10Q25L'), which(coln46 == 'B10Q25P'), by = 3))]) # looping round their aerobic exercise
  {
    if (j > 5) # selecting exercises which people do more than once a week
    {
      avd46$aer_s[x] = avd46$aer_s[x] + 1
    }
  }
}
...
for (x in which(avd46$sum >= 5.5)) # looping through people who do ~7 days of exercise a week
{
  ...
  for (j in avd46[x,c(seq(which(coln46 == 'B10Q25Y'),which(coln46 == 'B10Q25BB'), by = 3))]) # looping round their aerobic exercise
  {
    if (j > 5) # selecting exercises which people do more than once a week
    {
      avd46$team_s[x] = avd46$team_s[x] + 1
    }
  }
}
